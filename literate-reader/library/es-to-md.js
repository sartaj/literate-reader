...
/**
 * ### `isImageLine`
 * Image lines may require an extra space.
 *
 * _Added by sartaj_
 */
const isImageLine = /!\[/;
...
// that it's a `prototype` function, thus `flushFunction` and
...
const doc = file =>
	fs.createReadStream(file)
		...
		.pipe(new Transform({
			...
			// `#!/usr/bin/env node`.
			extname: path.extname(file).substr(1) || 'ts',
